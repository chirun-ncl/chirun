...
class ColorParser:
    ...
    def expr(self) -> Optional[Token]:
        ...
        if mix is not None:
            for m in mix['mixes']:
                mpct = m['pct']
                mcol = Color().makeColor('named', m['id'], self.colors, self.current_color).as_model(color.model)
...
class Color:
    ...
    def html(self) -> str:
        ...
        return '#%.2X%.2X%.2X' % (min(int(rgb.r * 255), 255),
                                  min(int(rgb.g * 255), 255),
                                  min(int(rgb.b * 255), 255))
...
class hsbColor(Color):
    ...
    def as_rgb(self) -> 'rgbColor':
        if self.s == 0.:
            return rgbColor(self.b, self.b, self.b)
        i = int(self.h * 6.)
        f = (self.h * 6.) - i
        p, q, t = self.b * (1. - self.s), self.b * (1. - self.s * f), self.b * (1. - self.s * (1. - f))
        i %= 6
        if i == 0:
            return rgbColor(self.b, t, p)
        if i == 1:
            return rgbColor(q, self.b, p)
        if i == 2:
            return rgbColor(p, self.b, t)
        if i == 3:
            return rgbColor(p, q, self.b)
        if i == 4:
            return rgbColor(t, p, self.b)
...
class ColorSeries(Color):
    ...
    def __repr__(self) -> str:
        if self.method == 'last':
            return ('<ColorSeries(model={}, current={}, base={}, last={})>'
                    .format(self.model, self.current, self.base, self.last))
        else:
            return ('<ColorSeries(model={}, current{}, base={}, step={})>'
                    .format(self.model, self.current, self.base, self.step))
...
class ColorEnvironment(Environment, ColorCommandClass):
    ...
    def invoke(self, tex) -> None:
        ...
        self.parser: ColorParser = ColorParser(u.getPath('packages/xcolor/colors'),
...
class color(ColorEnvironment):
    ...
    def source(self) -> str:
        ...
        return (r'\require{{color}}{{\color[rgb]{{{r:.15f}, {g:.15f}, {b:.15f}}}{children}}}'
                .format(r=rgb.r, g=rgb.g, b=rgb.b, children=sourceChildren(self)))
