...
# This is a pretty nasty hack. The argument parser breaks with lstlisting
# but can be fixed by passing an empty argument list instead of no arguments
# at all. This code looks at the next character in the stream to see if arguments
# are coming up and if not it inserts '[]' into the stream before parsing.
...
def _format(self, file):
    ...
    linenos = False
    if ('numbers' in list(self.attributes['arguments'].keys()) or 'numbers' in list(PackageOptions.keys())):
        linenos = 'inline'
